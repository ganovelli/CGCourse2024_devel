...
#include "..\common\gltf_loader_to_texture.h"
...
int main(int argc, char ** argv)
{
	...
	window = glfwCreateWindow(TEXTURE_WIDTH, TEXTURE_HEIGHT, "code_15_gpu_raytracing", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
...
//	glUniform1i(rt_shader["uNTriangles"], std::max<int> (  model.n_tri,   2) );
	check_gl_errors(__LINE__, __FILE__);
	float box4[4];
